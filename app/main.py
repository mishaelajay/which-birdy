""" Fastapi endpoint to serve the model """
import time
...
def index():
    ...
    return {
        'message':
            'Hello, Bird enthusiast. Checkout our docs page by appending /docs to the current url'
    }
...
def validate_urls(image_urls):
    ...
    for url in image_urls:
        if not ImageProcessor.is_url(url):
            raise HTTPException(
                status_code=422, detail='One of the urls you entered is invalid: %s' % url
            )
